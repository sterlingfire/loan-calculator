...
function getFormValues() {
  let result = {};
  result['amount'] = document.getElementById("loan-amount").value;
  result['years'] = document.getElementById("loan-years").value;
  result['rate'] = document.getElementById("loan-rate").value;
...
function calcMonthlyPayment(amount, years, rate) {
  if (amount === 0 || years === 0 || rate === 0) return "0";
...
function getFormValuesAndDisplayResults() {
  ...
  let monthly = calcMonthlyPayment(formValues.amount, formValues.years, formValues.rate);
  // limit to two digits
